...
with open(os.path.join(location, "camelCards.txt"), "r") as file:
    for line in file:
        ...
        bid = line.split(" ")[1]
        occurences = []
        iterCardTypes = iter(cardTypes)
        next(iterCardTypes)
        for type in iterCardTypes:
            occurences.append(hand.count(type))
        oldMaxOccurences, maxOccurences = max(occurences), max(occurences)
        if maxOccurences > 0:
            maxOccurences += hand.count("J")
        elif maxOccurences == 0:
            maxOccurences = 5
        if maxOccurences == 1:
        ...
        if maxOccurences == 3:
            if occurences.count(2) == 1:
                if oldMaxOccurences == maxOccurences:
